...
// ─────────────────────────────────────────────
class AppColors {
  static const Color primary = Color(0xFFE8272A);   // Crimson red
  ...
  Future<void> playList(List<MediaItem> items, int index) async {
    try {
      ...
      final sources = items.map((item) =>
        AudioSource.file(item.id, tag: item)
      ).toList();
          ...
          children: [
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                child: const Icon(CupertinoIcons.refresh, size: 18, color: AppColors.textSecondary),
              ),
            ),
          ],
        ),
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                child: const Icon(CupertinoIcons.music_note_2, size: 40, color: AppColors.primary),
              ),
      ...
      sliver: SliverList(
        delegate: SliverChildBuilderDelegate(
          (context, index) {
            ...
            final isAudio = name.endsWith('.mp3') || name.endsWith('.m4a') || name.endsWith('.aac') || name.endsWith('.opus');
    ...
    return Container(
      ...
      child: ListTile(
        ...
        leading: Container(
          ...
          child: Icon(
            isVideo ? CupertinoIcons.play_rectangle_fill : CupertinoIcons.music_note,
            color: AppColors.primary,
            size: 22,
          ),
        ),
      ...
      child: Column(
        ...
        children: [
          ...
          StreamBuilder(
            ...
            builder: (context, snapshot) {
              final tag = snapshot.data?.currentSource?.tag as MediaItem?;
              return Text(
                tag?.title ?? 'غير معروف',
                maxLines: 1,
                overflow: TextOverflow.ellipsis,
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              _controlBtn(CupertinoIcons.backward_fill, () => player.seekToPrevious()),
              ...
              const SizedBox(width: 24),
              _controlBtn(CupertinoIcons.forward_fill, () => player.seekToNext()),
            ],
          ),
        ],
      ),
            ...
            sliver: SliverList(
              delegate: SliverChildListDelegate([
                _BrowseCard(
                  ...
                  title: 'يوتيوب',
                  subtitle: 'تصفح وحمّل الفيديوهات والأصوات',
                  color: AppColors.primary,
                  bgColor: AppColors.redLight,
                  onTap: () {
                    Navigator.push(
                      context,
                      CupertinoPageRoute(builder: (_) => const YouTubeWebViewPage()),
                    );
                  },
                ),
                const SizedBox(height: 12),
                _BrowseCard(
                  ...
                  onTap: () {
                    Navigator.push(
                      context,
                      CupertinoPageRoute(builder: (_) => const LocalFilesPage()),
                    );
                  },
                ),
              ]),
...
class _YouTubeWebViewPageState extends State<YouTubeWebViewPage> {
  ...
  Future<void> _showDownloadOptions() async {
    final videoId = _extractVideoId(_currentUrl);
    if (videoId == null) {
      _showSnack('لا يوجد فيديو في الصفحة الحالية');
      return;
    }
    showModalBottomSheet(
      context: context,
      backgroundColor: Colors.transparent,
      builder: (_) => _DownloadSheet(videoId: videoId, onDownload: _download),
    );
  }

  Future<void> _download(String videoId, StreamManifest manifest, String quality, bool audioOnly) async {
    ...
    try {
      ...
      _showSnack('✅ تم التحميل: $fileName');
    } catch (e) {
      setState(() { _isDownloading = false; });
      _showSnack('❌ فشل التحميل: $e');
    }
  }
      ...
      appBar: AppBar(
        ...
        title: GestureDetector(
          onTap: () async {
            ...
            if (url != null) setState(() => _currentUrl = url);
          },
          child: Container(
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                _browserBtn(CupertinoIcons.chevron_back, () => _controller.goBack()),
                _browserBtn(CupertinoIcons.chevron_forward, () => _controller.goForward()),
                _browserBtn(CupertinoIcons.refresh, () => _controller.reload()),
                _browserBtn(CupertinoIcons.house_fill,
...
class _DownloadSheet extends StatefulWidget {
  ...
  final Function(String, StreamManifest, String, bool) onDownload;

  const _DownloadSheet({required this.videoId, required this.onDownload});
...
class _DownloadSheetState extends State<_DownloadSheet> {
      ...
      child: Column(
        ...
        children: [
          ...
          if (_loading)
            ...
            _downloadOption('🎵 صوت فقط (جودة عالية)', () {
              Navigator.pop(context);
              widget.onDownload(widget.videoId, _manifest!, 'high', true);
            }),
            const SizedBox(height: 8),
            _downloadOption('📹 فيديو جودة عالية', () {
              Navigator.pop(context);
              widget.onDownload(widget.videoId, _manifest!, 'high', false);
            }),
            const SizedBox(height: 8),
            _downloadOption('📹 فيديو جودة متوسطة (360p)', () {
              Navigator.pop(context);
              widget.onDownload(widget.videoId, _manifest!, 'medium', false);
            }),
          ],
  ...
  Widget _downloadOption(String label, VoidCallback onTap) {
    return GestureDetector(
      onTap: onTap,
      child: Container(
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: const Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      Icon(CupertinoIcons.folder_open, color: Colors.white, size: 20),
                      SizedBox(width: 10),
                      Text(
                        'تصفح الملفات',
...
class _SettingsPageState extends State<SettingsPage> {
  ...
  Future<void> _clearDownloads() async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('تم حذف جميع التنزيلات', textDirection: TextDirection.rtl),
          backgroundColor: AppColors.textPrimary,
          behavior: SnackBarBehavior.floating,
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
        ),
            ...
            sliver: SliverList(
              delegate: SliverChildListDelegate([
                ...
                Container(
                  ...
                  child: const Row(
                    children: [
                      Icon(CupertinoIcons.music_note_2, color: Colors.white, size: 32),
                      SizedBox(width: 12),
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('مستمع', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w700)),
                          Text('الإصدار 1.0.0', style: TextStyle(color: Colors.white70, fontSize: 13)),
                        ],
                      ),
                    ],
                  ),
                ...
                _settingsSection('التحميل', [
                  _infoTile('مسار التحميل', _downloadPath.split('/').last, CupertinoIcons.folder_fill),
                  _qualityTile(),
                ]),
                const SizedBox(height: 16),
  ...
  Widget _qualityTile() {
    return ListTile(
      ...
      leading: Container(
        ...
        child: const Icon(CupertinoIcons.dial_fill, color: AppColors.primary, size: 18),
      ),
      title: const Text('جودة التحميل', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: AppColors.textPrimary)),
      trailing: CupertinoSlidingSegmentedControl<String>(
        groupValue: _downloadQuality,
        thumbColor: AppColors.primary,
        children: const {
          'high': Padding(padding: EdgeInsets.symmetric(horizontal: 8), child: Text('عالية', style: TextStyle(fontSize: 11))),
          'medium': Padding(padding: EdgeInsets.symmetric(horizontal: 8), child: Text('متوسطة', style: TextStyle(fontSize: 11))),
        },
        onValueChanged: (v) {
          if (v != null) {
